...
struct PlanIncomeCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(income.tenge).font(.largeTitle.bold()).minimumScaleFactor(0.6)
            Text("\(sourceCount) planned source\(sourceCount == 1 ? "" : "s")")
                .font(.caption)
            Text("A plan for your income, not money received.")
...
#Preview("Plan income · multiple expected sources", traits: .sizeThatFitsLayout) {
    PlanIncomeCard(income: PlanPreviewData.income, sourceCount: 2, onEdit: {}).padding()
}
